...
#  {{ include "harness-manager,externalSecretEnv " (dict "secret" "foo" "default" "akljdlakfjd" "context" $ )}}
{{- define "harness-manager.externalSecretEnv" -}}
            - name: {{ .secret }}
            {{- if .context.Values.global.useExternalSecrets }}
              valueFrom:
                secretKeyRef:
                name: {{ .secret | lower }}
                key: value
...
{{/* 
{{ include "harness-manager.externalSecret" (dict "secret" "foo" "shared" "true" "context" $) }}
*/}}
{{- define "harness-manager.externalSecret" -}}
{{- if .context.Values.global.useExternalSecrets }}
...
spec:
  ...
  secretStoreRef:
    ...
    {{- if .shared }}
    name: gcr-shared-secrets
    {{- else }}
    name: project-shared-secrets
    {{- end }}
  target:
    template:
      type: Opaque
